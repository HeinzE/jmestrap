...
/// GET / — embedded dashboard (dashboard.html compiled in)
async fn dashboard() -> Html<&'static str> {
    Html(include_str!("../dashboard.html"))
}

/// GET /ui — permanent redirect to / (kept for historical bookmarks)
async fn dashboard_ui_redirect() -> Redirect {
    Redirect::permanent("/")
}
...
pub fn router(state: AppStateRef) -> Router {
    let r = Router::new()
        .route("/", get(dashboard))
        .route("/ui", get(dashboard_ui_redirect))
        .route("/ping", get(ping))
...
mod tests {
    ...
    #[tokio::test]
    async fn test_dashboard_root_route() {
        let app = app();
        let req = Request::builder()
            .method("GET")
            .uri("/")
    ...
    #[tokio::test]
    async fn test_dashboard_ui_redirects_to_root() {
        let app = app();
        let req = Request::builder()
            .method("GET")
            .uri("/ui")
    ...
    #[tokio::test]
    async fn test_list_recordings_returns_newest_first() {
        let app = app();
        ...
        for desc in ["first", "second", "third"] {
            let create = Request::builder()
                ...
                .uri("/recordings")
                .header("content-type", "application/json")
                .body(Body::from(json!({"description": desc}).to_string()))
                .unwrap();
        ...
        let refs: Vec<_> = body["recordings"]
            .as_array()
            .unwrap()
            .iter()
            .map(|r| r["reference"].as_u64().unwrap())
            .collect();
